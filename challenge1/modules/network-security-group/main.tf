...
# Resource : ald-tc-idf-dev-we-nsg
...
resource "azurerm_network_security_rule" "custom_rules_prefixes" {
  count                        = "${length(var.custom_rules_prefixes)}"
  name                         = "${lookup(var.custom_rules_prefixes[count.index], "name", "default_rule_name")}"
  priority                     = "${lookup(var.custom_rules_prefixes[count.index], "priority")}"
  direction                    = "${lookup(var.custom_rules_prefixes[count.index], "direction", "Any")}"
  access                       = "${lookup(var.custom_rules_prefixes[count.index], "access", "Allow")}"
  protocol                     = "${lookup(var.custom_rules_prefixes[count.index], "protocol", "*")}"
  source_port_ranges           = "${split(",", replace(  "${lookup(var.custom_rules_prefixes[count.index], "source_port_range", "*" )}"  ,  "*" , "0-65535" ) )}"
  destination_port_ranges      = "${split(",", replace(  "${lookup(var.custom_rules_prefixes[count.index], "destination_port_range", "*" )}"  ,  "*" , "0-65535" ) )}"
  source_address_prefixes      = "${split(",",lookup(var.custom_rules_prefixes[count.index], "source_address_prefixes", "0.0.0.0/0"))}"
  destination_address_prefixes = "${split(",",lookup(var.custom_rules_prefixes[count.index], "destination_address_prefixes", "0.0.0.0/0"))}"
  description                  = "${lookup(var.custom_rules_prefixes[count.index], "description", "Security rule for ${lookup(var.custom_rules_prefixes[count.index], "name", "default_rule_name")}")}"
  resource_group_name          = "${var.resource_group_name}"
  network_security_group_name  = "${azurerm_network_security_group.nsg.name}"
}
